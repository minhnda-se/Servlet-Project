...
public class SelectCategoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<ProductDTO> tempProductList = dao.getProductList();
            String category = request.getParameter("category");
            if (category == null || category.isEmpty()){
                productList = dao.getProductList();
            }
            else {
                productList = dao.getProductListByCategory(category, tempProductList);
            }
            session.setAttribute("PRODUCT_LIST", productList);
            session.setAttribute("URL", url);
            request.setAttribute("ADMIN_ACTION", "mystore");
        }
        finally {
