    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (count == null || !count.equals("1")){
                ...
                ItemDTO item = cartDAO.getItemFromCart(userId, gameId);
                boolean check = false;
                if (item == null) {
                    check = cartDAO.addToCart(1, gameId, userId);
                } else {
                    int quantity = item.getQuantity() + 1;
                    check = cartDAO.updateCart(quantity, userId, gameId);
                }
                if (check) request.setAttribute("ADD_CART_MESSAGE", "Adding successfully!");
                else request.setAttribute("ADD_CART_MESSAGE", "Adding failed!");
